...
def format_findings_summary(findings: List[Dict[str, Any]], audit_type: str = "service") -> str:
    ...
    if critical_findings:
        summary += f"🚨 **{len(critical_findings)} Critical Issues** (require immediate attention)\n"
    if warning_findings:
...
def create_targeted_audit_request(
    ...
    if selected_finding_index < 1 or selected_finding_index > len(findings):
        raise ValueError(f"Invalid finding index {selected_finding_index}. Must be between 1 and {len(findings)}")
    ...
    if not targeted_targets:
        ...
        logger.warning(f"Could not find exact target match for finding {selected_finding_index}, creating new target")
        if audit_type == "service":
            targeted_targets = [{
                "Type": "service",
                "Data": {
